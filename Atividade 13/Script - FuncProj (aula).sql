create database FuncProj;
use FuncProj;
drop database FuncProj;
...
salario decimal (7,2),
sexo char(1),
CONSTRAINT chksexo  check (sexo='m' or sexo='f' or sexo='n'),
fkSupervisor int,
dataNasc DATE,
fkDepto int,
...
alter table Departamento add constraint departamento_fkGerente foreign key (fkGerente)
references Funcionario (idFunc);

desc Departamento;

alter table Funcionario add constraint funcionario_fkSupervisor foreign key (fkSupervisor)
references Funcionario (idFunc);
...
-- exibir funcionarios e seus departamentos correspondentes, de forma que apareçam todos nulos
...
select idProj, nomeProj,
	(select count(fkFunc) from FuncProj where fkProj = idProj  group by fkProj)
    as 'Quantidade de funcionario'
     from projeto;
